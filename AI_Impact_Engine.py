...
st.markdown("""
    <h1 style='text-align:center;'>👑 AI ROI Predictor</h1>
    <p style='text-align:center;'>End-to-end ML Pipeline Dashboard</p>
...
with st.form("input_form"):
    ...
    with col1:
        ...
        revenue = st.number_input("Annual Revenue (M USD)", value=150.0)
    ...
    with col2:
        ...
        failure_rate = st.slider("AI Failure Rate (%)", 0.0, 100.0, 15.0)

    with col3:
        primary_tool = st.selectbox("AI Tool", ["ChatGPT", "Claude", "Gemini", "Custom"])
        num_tools = st.number_input("Number of AI Tools", min_value=1, value=5)
        active_projects = st.number_input("Active Projects", value=3)
        budget = st.slider("AI Budget (%)", 0.0, 100.0, 12.0)
...
if submit:
    ...
    try:
        ...
        with col1:
            ...
            st.metric("Predicted ROI", f"{prediction:.2%}")

            investment = revenue * (budget / 100)
            gain = investment * prediction

            st.write(f"Investment: ${investment:.2f}M")
            st.write(f"Estimated Gain: ${gain:.2f}M")

        with col2:
            st.subheader("🏢 AI Maturity Check")
            ...
            if failure_rate > 25:
                st.error("High Risk: Failure rate too high")
            else:
                st.success("Operational Risk Controlled")

            if maturity > 70:
                st.info("AI Leader")
            else:
                st.info("AI Adopter Stage")
        ...
        with c1:
            st.info("Model trained using XGBoost + GridSearchCV pipeline")

        with c2:
            st.warning("Expected error range: ±0.7 ROI")

        with c3:
            if revenue > 1000:
                st.error("Out-of-distribution input")
            else:
